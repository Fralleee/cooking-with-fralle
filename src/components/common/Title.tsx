...
const TitleText = styled.h1<{ fontSize: number, divider?: number, absolute?: boolean }>`
  ...
  ${props => props.divider ?
    `font-size: clamp(${props.fontSize / props.divider}px, calc(${6 / props.divider}vw + ${1.25 / props.divider}rem}), ${12 / props.divider}rem); `
...
const Title: FC<TitleProps> = ({ children, color, subtitle }) => {
  ...
  return (
    <TitleText color={color} fontSize={fontSize}>
      {children}
      {subtitle && <TitleText fontSize={fontSize} divider={4} absolute>{subtitle}</TitleText>}
    </TitleText>
  )
}
